...
L_me=60.2   *10**(-2)   
a_al=0.52   *10**(-2)
a_al=1.199  *10**(-2)   #von Nicolai  Mit den Werten von Nicolai, kommen auch vernünftige Ergebnisse raus
a_fe=0.505  *10**(-2)  
a_fe=1.199  *10**(-2)   #von Nicolai
R_me=0.52/2 *10**(-2) 
...
plt.xlabel(r"$\eta/\;\si{\cubic\metre}$")
plt.ylabel(r"$D(\eta)/\;\si{\metre}$")
plt.legend(loc='best')
plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
plt.tight_layout()
