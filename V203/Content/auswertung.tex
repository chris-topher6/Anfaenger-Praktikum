...
\begin{longtable}{S S}
    %\centering
    \caption{Messwerte der Temperatur im niedrigen Druckbereich}\\
    \label{tab:niedringe}\\
      %\sisetup{table-format=7.2}
          \toprule
          {$p / \si{\pascal}$} & {$ T / \si{\kelvin}$} \\
...
Auf Grundlage der Messwerte in Tabelle \ref{tab:niedringe} wird nun die Verdampfungswärme $L$ des
...
\end{equation}
verwendet. $p_0=\SI{100000}{\pascal}$ ist dabei der Umgebungsdruck und $R$ die allgemeine Gaskonstante.
Wird nun $x=1/T$ und $y=ln(p-p_0)$ gewählt, ergibt sich die lineare Gleichung:
\begin{equation}
  y=-\frac{L}{R}\cdot x+c \label{eqn:dampfdruck2}.
\end{equation}
Werden die Messwerte nun in ein Koordinatensystem eingetragen und wird wie oben beschrieben $x=1/T$ und $y=ln(p-p_0)$ eingesetzt,
...
\begin{align*}
  ...
  c           & =\num{18.03 \pm 0.54}
\end{align*}
bei hohem Druck. Die Unsicherheit der Parameter wurden genau wie die Regression mit \textit{numeric python}
berechnet.
\\
...
Um die Verdampfungswärme $L$ in Abhängigkeit von der Temperatur zu bestimmen, wird die
Clausius-Clapeyronsche Gleichung \eqref{eqn:clausiusclapeyron} nach L umgeformt. Dann ergibt sich
\begin{equation}
...
\ref{fig:Polynom} wurde dieses Polynom mit Python graphisch dargestellt.
\begin{figure}
  \centering
