...
while j < 5:
    #dQ/dt=-k*A*(Ti-Tj)/dx
    dQ=np.array([-k_messing*A_messingb*(T1[i[j]]-T2[i[j]])/d_x, -k_messing*A_messings*(T3[i[j]]-T4[i[j]])/d_x,-k_aluminium*A_aluminium*(T5[i[j]]-T6[i[j]])/d_x,  -k_edelstahl*A_edelstahl*(T7[i[j]]-T8[i[j]])/d_x])
    j=j+1
    print(dQ)
