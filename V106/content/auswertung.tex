\section{Auswertung}
  ...
  \subsection{Freie Schwingungen}
    ...
    und Pedel 2 durchgeführt. Die Unsicherheit wurde nach Kapitel \ref{sec:Fehlerrechnung} bestimmt, indem zuerst der Mittelwert der Messwerte
    und dann die Standardabweichung der Messwerte von dem Mittelwert berechnet wurde.
    ...
    Dies wird in der Diskussion abschließend behandelt. Bei obigen Messungen hing die Feder $0.288$m unter der Pendelaufhängung.
  \subsection{gleich- und gegensinnige Schwingungen}
    \begin{table}
      \centering
        ...
        \begin{tabular}{S S S S S S S}
          \toprule
          {Pendellänge/m} & {$T_{+}/\si{\second}$} & {$\increment T_{+}/\si{\second}$} & {$T_{-}/\si{\second}$} & {$\increment T_{-}/\si{\second}$} & {$T_{-}/\si{\second}$ Versuch 2} &
          {$\increment T_{-}/\si{\second}$ Versuch 2} \\
          \midrule
    ...
    erkennbar. Die Unsicherheiten sind analog zu Abschnitt \ref{sec:freieschwingungen} berechnet worden.
  ...
  Die beiden Kopplungsgrade wurden auf Basis der Messwerte für gleich- und gegensinnige Schwingungen (siehe \ref{tab:aufgabe23}) sowie Gleichung \eqref{tab:aufgabe23}
  berechnet. Wir haben hier für die Pendellänge von $0.993$m auch einmal die fehlerbehaftete Messung der
  ...
  der Aufhängung des Pendels und bei einer Pendellänge von $0.325$m bei $0.538$m unter der Aufhängung.
    \begin{table}
      \centering
        \caption{Der Kopplungsgrad $K$ für zwei verschiedene Pendellängen}
          ...
          \begin{tabular}{S S S S S}
            \toprule
            {Pendellänge/m} & {$K$}  & {$\increment K$} & {$K$ Versuch 2} & {$\increment K$ Versuch 2}\\
            \midrule
        ...
        wie man gut an $K$ Versuch 2 sehen kann. Die Unsicherheit wurde hier nach der Gaußschen Fehlerfortpflanzung, wie in Abschnitt \ref{sec:Fehlerrechnung}
    ...
    \subsection{Schwebungsdauer der gekoppelten Pendel}
      ...
      sehr lange und die Unsicherheit ist so hoch. Die anderen Werte sind jedoch sehr ähnlich, die Messungen scheinen keine systematischen Fehler zu enthalten.
      ...
      siehe dazu Abschnitt \ref{sec:Fehlerrechnung}.
