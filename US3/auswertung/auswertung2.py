...
d, v_stroem45, I45       = np.genfromtxt('b-45%.dat',    unpack=True) #45 und 70 sind im Code leider komplett vertauscht
d, v_stroem70, I70       = np.genfromtxt('b-70%.dat',    unpack=True) #In der Ausgabe und in den Plots aber richtig
...
print(f'a70V    = {params45V0   :.4}    \t m/s^2  ')
print(f'b70V    = {params45V1   :.4}    \t m/s    ')
print(f'a70Vneu = {params45V0neu:.4} \t\t m/s^2  ')
print(f'b70Vneu = {params45V1neu:.4} \t m/s    ')
print(f'a45V    = {params70V0   :.4}    \t\t m/s^2  ')
print(f'b45V    = {params70V1   :.4}    \t\t m/s    ')
print(f'a70I    = {params45I0   :.4}    \t V^2/s^2')
print(f'b70I    = {params45I1   :.4}    \t V^2/s  ')
print(f'a70Ineu = {params45I0neu:.4} \t V^2/s^2')
print(f'b70Ineu = {params45I1neu:.4} \t V^2/s  ')
print(f'a45I    = {params70I0   :.4}    \t V^2/s^2')
print(f'b45I    = {params70I1   :.4}    \t V^2/s  ')
...
plt.plot(x,     gerade(x,     *params45V),     'gray',  label="Regression 70%")
plt.plot(xvneu, gerade(xvneu, *params45Vneu),  'r',     label="bereinigt  70%")
plt.plot(x,     gerade(x,     *params70V),     'g',     label="Regression 45%")
plt.plot(d,     v_stroem45,                    'r.',    label='Messdaten  70%')
plt.plot(d[0],  v_stroem45[0],                 'r*',    label='Ausreißer  70%')
plt.plot(d[1],  v_stroem45[1],                 'r*')    
plt.plot(d,     v_stroem70,                    'g.',    label='Messdaten  45%')
plt.xlabel(r"$d [s]$")
...
plt.figure()#v_stroem
plt.plot(x, gerade(x, *params45I),            'gray',  label="Regression 70%")
plt.plot(xIneu, gerade(xIneu, *params45Ineu), 'r',     label="bereinigt  70%")
plt.plot(x, gerade(x, *params70I),            'g',     label="Regression 45%")
plt.plot(d,     I45,                          'r.',    label='Messdaten  70%')
plt.plot(d[7],  I45[7],                       'r*',    label='Ausreißer  70%')
plt.plot(d,     I70,                          'g.',    label='Messdaten  45%')
plt.xlabel(r"$d [s]$")
