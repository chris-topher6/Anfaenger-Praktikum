...
params15s, cov15s = np.polyfit(v_pump, doppler(f15s, a15), deg=1, cov=True) #Parameter berechnen
errs15s     = np.sqrt(np.diag(cov15s))
...
x15=np.linspace(np.min(v_pump), np.max(v_pump))
plt.plot(x15, gerade(x15, *params15s), 'k', label="Regression")
#plt.plot(doppler(f15s, a15), stroemung(f15s, a15), 'r.', label='Messdaten')
plt.plot(v_pump, doppler(f15s, a15), 'r.', label='Messdaten')
#plt.xlabel(r"$\Delta\nu/cos(\pi/12)$")
#plt.ylabel(r"v [m/s]")
...
x30=np.linspace(np.min(v_pump), np.max(v_pump))
plt.plot(x30, gerade(x30, *params30s), 'k', label="Regression")
#plt.plot(doppler(f30s, a30), stroemung(f30s, a30), 'g.', label='Messdaten')
plt.plot(v_pump, doppler(f30s, a30), 'g.', label='Messdaten')
#plt.xlabel(r"$\Delta\nu/cos(\pi/6)$")
#plt.ylabel(r"v [m/s]")
plt.xlabel(r"$v_{pump} [rpm]$")
plt.ylabel(r"$\Delta\nu/cos(\pi/6)$")
...
x45=np.linspace(np.min(v_pump), np.max(v_pump))
plt.plot(x45, gerade(x45, *params45s), 'k', label="Regression")
#plt.plot(doppler(f45s, a45), stroemung(f45s, a45), 'b.', label='Messdaten')
plt.plot(v_pump, doppler(f45s, a45), 'b.', label='Messdaten')
#plt.xlabel(r"$\Delta\nu/cos(\pi/4)$")
#plt.ylabel(r"v [m/s]")
plt.xlabel(r"$v_{pump} [rpm]$")
plt.ylabel(r"$\Delta\nu/cos(\pi/4)$")
